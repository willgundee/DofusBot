...
namespace GofusSharp
{
    public class Sort
    {
        ...
        public int TourAvantRelance { get; internal set; }
        public int NbLancerDuTour { get; internal set; }
        internal Sort(Gofus.Sort sort)
        {
            ListEffets = new Liste<Effet>();
            foreach (Gofus.Effet effet in sort.LstEffets)
            {
                ListEffets.Add(new Effet(effet));
            }
            Nom = sort.Nom;
            LigneDeVue = sort.LigneDeVue;
            PorteeModifiable = sort.PorteeModifiable;
            CelluleLibre = sort.CelluleLibre;
            ZonePortee = new Zone(sort.ZonePortee);
            ZoneEffet = new Zone(sort.ZoneEffet);
            TauxDeRelance = sort.TauxDeRelance;
            CoutPA = sort.PointActionRequis;
            if (System.Enum.IsDefined(typeof(nom_sort), RemoveDiacritics(sort.Nom.Replace(' ', '_').Replace('\'', '_').ToLower())))
                VraiNom = (nom_sort)System.Enum.Parse(typeof(nom_sort),RemoveDiacritics(sort.Nom.Replace(' ', '_').Replace('\'', '_').ToLower()));
            else
                VraiNom = nom_sort.sort_monstre;
